 ...
 * Iterador interno. Recorre la lista e invoca la funcion con cada elemento de
 * la misma como primer parámetro. Dicha función puede devolver true si se deben
 * seguir recorriendo elementos o false si se debe dejar de iterar elementos y en ese caso elimina el elemento.
 *
 * El puntero aux se pasa como segundo argumento a la función del usuario.
 *
 * La función devuelve el elemento eliminado o NULL en caso de error
 * (errores de memoria, función o lista NULL, etc).
 *
 */
void *lista_buscar_y_eliminar(lista_t* lista, bool(*funcion)(void*, void*), void *aux);
